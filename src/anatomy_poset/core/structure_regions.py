...
def filter_structures_for_regions(
    structures: List,
    *,
    use_all: bool,
    selected_region_ids: Set[str],
    ...
    """
    Return (filtered_structures, warnings).

    If ``use_all``, returns a shallow copy of ``structures`` and no warnings.

    Otherwise keeps structures whose ``name`` appears in the union of selected regions,
    preserving input order. Warns about dropped names (not in the union of selected regions).
    """
    if use_all:
        return list(structures), []

    allowed = union_region_names(selected_region_ids)
    all_catalog = REGION_1_TRUNK_VISCERA_NO_ARMS | REGION_2_NEUROAXIS_SHOULDER_ARMS | REGION_3_LUMBPELVIS_LEGS
    ...
    if dropped:
        warnings.append(
            "Omitted (not in selected region union): " + ", ".join(sorted(set(dropped)))
        )
        unknown = sorted({n for n in dropped if n not in all_catalog})
        if unknown:
            warnings.append(
                "These names are not in the 3 preset atlas lists at all: " + ", ".join(unknown)
            )
